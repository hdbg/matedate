...
export async function loadProfile(): Promise<ProfileData | null> {
  ...
  // Persona names label solo/practice rows ("AI date · Maya").
  const personaIds = [...new Set(solos.map((s) => s.persona_id).filter((id): id is string => !!id))];
  ...
  const counts: ProfileData["counts"] = { all: 0, solo: 0, ranked: rankedCount ?? 0, practice: 0, review: 0, puzzle: 0 };
  for (const item of history) counts[item.category] += 1;
  counts.all = history.length + counts.ranked;
