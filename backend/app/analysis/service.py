...
from .transcript import Transcript, load_game_transcript, load_match_transcript
...
async def process_job(
    ...
    try:
        ...
        # The source: a game, or one side of a PvP match (request_match_analysis enqueues both).
    ...
    try:
        if game_id is not None:
            ...
            transcript = await load_match_transcript(db, match_id, side)
        result = await engine.analyze(transcript)
        validate_verdict(result.verdict, transcript)
        await _persist_analysis(
            db, job_id, transcript, result, analysis_id, game_id=game_id, match_id=match_id, side=side
        )
